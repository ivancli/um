...
trait UMRoleTrait
{
    ...
    public function cachedPermissions()
    {
        ...
        if (Cache::getStore() instanceof TaggableStore) {
            return Cache::tags(Config::get('um.permission_role_table'))->remember($cacheKey, Config::get('cache.ttl', 60), function () {
    ...
    {
        return $this->belongsToMany(Config::get('auth.providers.users.model'), Config::get('um.role_user_table'), Config::get('um.role_foreign_key'), Config::get('um.user_foreign_key'));
        ...
        if (is_array($name)) {
            ...
            foreach ($this->cachedPermissions() as $permission) {
                if ($permission->name == $name) {
                    return true;
                } elseif ($permission->childPermissions()->count() > 0) {
                    foreach ($permission->childPermissions as $childPermission) {
                        if ($childPermission->name == $name) {
                            return true;
                        }
                    }
                }
